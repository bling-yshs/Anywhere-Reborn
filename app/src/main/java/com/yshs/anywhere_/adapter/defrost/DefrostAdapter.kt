package com.yshs.anywhere_.adapter.defrost
  ...
  override fun convert(holder: BaseViewHolder, item: DefrostItem) {
    ...
    button.apply {
      ...
      when (item.mode) {
        Const.DEFROST_MODE_DSM -> {
          if (DSMClient.getDelegatedScopes(context)
              .contains(DevicePolicyManager.DELEGATION_PACKAGE_ACCESS)
          ) {
            isEnabled = false
    ...
    holder.apply {
      ...
      itemView.setOnClickListener {
        if (mPosition != -1) {
          val oldRadio: MaterialRadioButton =
            getViewByPosition(mPosition, R.id.radio) as MaterialRadioButton

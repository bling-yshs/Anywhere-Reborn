package com.yshs.anywhere_.adapter.applist
...
class AppListAdapter(mode: Int) :
  BaseQuickAdapter<AppListBean, BaseViewHolder>(R.layout.item_app_list), PopupTextProvider {
          ...
          item.isLaunchActivity -> {
            holder.itemView.rootView.setBackgroundColor(
              ContextCompat.getColor(
                context,
                R.color.launcher_activity_background
              )
            )
          }
          item.isExported -> {
      ...
      MODE_ICON_PACK -> {
        if (item.packageName == Const.DEFAULT_ICON_PACK) {
          holder.setImageDrawable(
            R.id.iv_app_icon,
            UxUtils.getAppIcon(context, context.packageName)
          )
        } else {
